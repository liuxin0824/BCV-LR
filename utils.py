...
def create_dynamics_models(
    ...
    obs_depth = 3
    idm_in_depth = obs_depth * (2 + config.ADD_TIME_HORIZON)  #ADD_TIME_HORIZON defaul 1
...
def eval_latent_repr(labeled_data: data_loader.DataStager, idm: IDM, env_name):
    batch = labeled_data.td_unfolded[:131072]
    actions = idm.label_chunked(batch).select("ta", "la").to(config.DEVICE)
    return train_decoder(data=actions, env_name = env_name)
...
def train_decoder(
    ...
    """
    TA_DIM = env_utils.ta_dim[env_name]
...
def eval_latent_repr_stage3(labeled_data: data_loader.DataStager, idm: IDM, decoder):
    batch = labeled_data.td_unfolded[:131072]
    actions = idm.label_chunked(batch).select("ta", "la").to(config.DEVICE)
    return train_decoder_stage3(data=actions, decoder = decoder)


def train_decoder_stage3(
    data: TensorDict,  # tensordict with keys "la", "ta"
    decoder,
    hidden_sizes=(128, 128),
    epochs=3,
...
class RandomShiftsAug_nobinliear(nn.Module):
