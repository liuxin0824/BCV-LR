...
TRAIN_CHUNK_LEN = 100000
TEST_CHUNK_LEN = 10000

DATA_LIMITATION = 5000


def _create_tensordict(length: int, obs_depth, env_name) -> TensorDict:
    return TensorDict(
        {
            "obs": torch.zeros(length, obs_depth, 64, 64, dtype=torch.uint8),
            "ta": torch.zeros(length, env_utils.ta_dim[env_name]),
            "done": torch.zeros(length,1),
            "rewards": torch.zeros(length,1),
            "ep_returns": torch.zeros(length,1),
...
class DataStager:
    def __init__(
        ...
        self.td = _create_tensordict(self.chunk_len * len(self.files), self.obs_depth, env_name)
    ...
    def get_iter(
        ...
        dataloader = DataLoader(
            self.td_unfolded,    # [:DATA_LIMITATION-1],  #正常
...
def load(env_name: str) -> tuple[DataStager, DataStager]:
    with doy.status(f"Loading expert data for env: {env_name}"):
        ...
        return _load(env_name, test=False), _load(env_name, test=False)
